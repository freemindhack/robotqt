...
QString Xml::readText(QString text, QString nodeName,QString rootXML)
{

    QDomDocument doc;

    if (!doc.setContent(text)) return QString("");

    QDomElement resultNode = doc.documentElement();
    QDomElement currentNode = doc.documentElement().firstChildElement();
    int i=0,j=0;
    while (i<resultNode.childNodes().length()) {
        if(currentNode.tagName()!=rootXML){
           currentNode = currentNode.nextSiblingElement();
        }else{
            QDomElement sencondNode=currentNode.firstChildElement();
            while (j<currentNode.childNodes().length()) {
                if(sencondNode.tagName()!=nodeName){
                   sencondNode = sencondNode.nextSiblingElement();
                }else{
                   resultNode=sencondNode;
                }
                j++;
            }
        }
